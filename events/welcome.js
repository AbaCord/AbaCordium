...
const welcomeMessages = [ // 50 KI genererte meldinger
...
module.exports = {
    ...
    async execute(member,client) {
        ...
        if (channel) {
            try {
                const welcomeText = welcomeMessages[Math.floor(Math.random() * welcomeMessages.length)].replace(/<userID>/g, `<@${member.id}>`);
